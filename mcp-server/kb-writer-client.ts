...
export type ApprovedCapabilityName = typeof APPROVED_CAPABILITY_NAMES[number];
export type JsonPrimitive = string | number | boolean | null;
export type JsonValue = JsonPrimitive | JsonValue[] | { [key: string]: JsonValue };
export type CapabilityArguments = Record<string, JsonValue | undefined>;
...
export class KbWriterBackendError extends Error {
  ...
  constructor(status: number, details: JsonValue) {
...
function requestSpec(name: ApprovedCapabilityName, args: CapabilityArguments): RequestSpec {
  ...
  switch (name) {
    ...
    case 'remove_manifest_source':
      return post(
        `${workspacePath()}/manifest/sources/${segment(required(args, 'source_id'))}/remove`,
        { expectedManifestId: required(args, 'expected_manifest_id') },
        idempotencyKey(),
      );
    case 'replace_manifest_source':
      return post(
        `${workspacePath()}/manifest/sources/${segment(required(args, 'source_id'))}/replace`,
        {
          expectedManifestId: required(args, 'expected_manifest_id'),
    ...
    case 'remove_manifest_item':
      return post(
        `${workspacePath()}/manifest/items/${segment(required(args, 'item_key'))}/remove`,
        { expectedManifestId: required(args, 'expected_manifest_id') },
        idempotencyKey(),
      );
    ...
    case 'select_update_target':
      return post(
        `${workspacePath()}/manifest/items/${segment(required(args, 'item_key'))}/select-target`,
        {
          expectedManifestId: required(args, 'expected_manifest_id'),
    ...
    case 'accept_manifest_risk':
      return post(
        `${workspacePath()}/manifest/items/${segment(required(args, 'item_key'))}/accept-risk`,
        {
          expectedManifestId: required(args, 'expected_manifest_id'),
    ...
    case 'resume_preview_job':
      return post(
        `/v1/intent-workspaces/preview-jobs/${segment(required(args, 'preview_job_id'))}/resume`,
        { originalManifestId: required(args, 'original_manifest_id') },
        idempotencyKey(),
      );
...
function post(path: string, body: JsonValue, idempotencyKey: JsonValue): RequestSpec {
  return { method: 'POST', path, body, idempotencyKey };
}

function required(args: CapabilityArguments, key: string): JsonValue {
  const value = args[key];
  if (value === undefined) throw new Error(`Missing required tool argument: ${key}`);
  return value;
}

function requiredObject(args: CapabilityArguments, key: string): Record<string, JsonValue> {
  const value = required(args, key);
  if (!isJsonObject(value)) throw new Error(`Tool argument ${key} must be an object`);
  return value;
}

function segment(value: JsonValue): string {
...
function queryParams(
  ...
  for (const [argumentName, queryName] of mappings) {
    ...
    if (value !== undefined && value !== null) params.set(queryName, String(value));
  }
  return params;
}

async function backendError(response: Response): Promise<KbWriterBackendError> {
  const contentType = response.headers.get('content-type') ?? '';
