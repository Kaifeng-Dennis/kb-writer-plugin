...
- Complete the Codebase Project Confirmation Gate before the first `qa_codebase` call in a conversation. Reuse the confirmed list afterwards; re-run the gate only when the resolved project set changes, the registry source changes, or the PM asks to change it.
