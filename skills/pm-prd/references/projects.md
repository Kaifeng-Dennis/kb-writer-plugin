...
Run this gate once per conversation, before the first `qa_codebase` request. Reuse `confirmed_project_list` for subsequent calls in the same conversation.

Re-run the gate only when one of these is true:

- The resolved project set changes, for example a later stage turns out to span an additional surface.
- The registry source changes between `api` and `fallback_snapshot`.
- The PM asks to change the project list.

An unchanged recommended list does not require re-confirmation.
